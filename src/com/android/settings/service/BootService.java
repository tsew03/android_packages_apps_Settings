...
import com.android.settings.VoltageControlSettings;
	...
	public void onStart(Intent intent, int startId) {
		...
		bootThread = new Thread() {
			...
			public void run() {
				...
				if (preferences.getBoolean(VoltageControlSettings.KEY_APPLY_BOOT, false)) {
					...
					Log.i(TAG, logInfo);
					new CMDProcessor().su.runWaitFor("busybox echo " + sb.toString() + " > " + VoltageControlSettings.MV_TABLE0);
					if (new File(VoltageControlSettings.MV_TABLE1).exists()) {
						new CMDProcessor().su.runWaitFor("busybox echo " + sb.toString() + " > " + VoltageControlSettings.MV_TABLE1);
					}
				}
			}
		};
